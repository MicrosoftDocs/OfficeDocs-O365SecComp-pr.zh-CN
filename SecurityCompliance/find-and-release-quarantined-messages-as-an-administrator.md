...
已隔离邮件在 EAC 的 "**隔离**" 页面上列出。默认情况下, "**接收**时间" 字段中的邮件按从最新到最旧的顺序排列。还会为每个邮件列出 "**发件人**"、"**主题**" 和 "**过期**" 值。您可以通过单击其标题对这些字段中的任何字段进行排序。如果第二次单击列标题, 则排序次序是相反的。"**隔离**" 页面最多显示500个邮件。 
...
- 您需要先分配权限, 然后才能执行此过程或过程。若要查看所需的权限, 请参阅[Exchange Online 中的功能权限](http://technet.microsoft.com/library/15073ce1-0917-403b-8839-02a2ebc96e16.aspx)主题中的 "隔离" 条目。 
...
2. 在 "**高级搜索**" 窗口中, 选择下列条件的任意组合。选中关联的复选框以启用每个条件。不支持通配符。 
    
1. **邮件 ID**您可以使用此参数对特定邮件执行目标搜索。例如, 如果您的组织中的用户发送或预定某一特定的邮件, 但它永远无法到达其目标, 则可以使用邮件跟踪功能搜索邮件。有关详细信息, 请参阅[运行邮件跟踪和查看结果](http://technet.microsoft.com/library/74a9fc59-7e0e-4832-baf9-2a86418b0079.aspx)。如果发现邮件已发送到隔离, 可能是因为它匹配某个规则或被标识为垃圾邮件, 那么您可以通过指定其邮件 ID 轻松地在隔离中找到此邮件。确保包含完整的邮件 ID 字符串。这可能包括尖括号 (\<\>)。 
...
1. 在 "**隔离**" 页面上, 选择特定的邮件, 屏幕右侧的详细信息窗格中将显示该邮件的属性摘要。 
  ...
  - **查看邮件头**单击此链接可打开 "**邮件头**" 对话框, 您可以通过该对话框查看邮件头文本。您还可以将邮件头文本复制到剪贴板, 并将其粘贴到[邮件头分析器](https://testconnectivity.microsoft.com/?tabid=mha)中。进入邮件头分析器工具后, 单击 "**分析邮件**头" 以检索有关邮件头的信息。 
...
3. 在 "**释放邮件**" 对话框中, 选择下列选项之一: 
  ...
  - **向所有收件人释放邮件**选择此选项时, 请注意, 不能将一封邮件释放到同一收件人的一次。如果收件人之前已收到邮件, 则不会再次向该收件人释放邮件。 
    
  - **将邮件释放给指定的收件人**选择可将邮件释放到的收件人。由于一封邮件只能发布给每个收件人一次, 因此该列表中只会显示可释放的收件人。支持多选。做出选择后, 单击 "**添加**"。
...
2. 单击邮件以将其选中, 或使用 shift 键选择多封邮件。然后单击 "**释放邮件**" 图标。 
    
3. 从下拉列表中单击 "**将所选邮件释放给所有收件人**"。 
    
4. 将打开 "警告" 对话框。阅读警告, 如果要继续, 请选择 **"是"** 。选择此选项时, 请注意, 不能将一封邮件释放到同一收件人的一次。如果收件人之前已收到邮件, 则不会再次向该收件人释放邮件。 
