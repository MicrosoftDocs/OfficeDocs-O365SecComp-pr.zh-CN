...
高级垃圾邮件筛选选项使管理员能够检查的一条消息的各种内容属性。在邮件中的这些属性的状态会增加 （从而提高其标识为垃圾邮件的可能性） 的邮件的垃圾邮件得分，或将邮件标记为垃圾邮件。ASF 选项面向特定的邮件属性，如 HTML 标记和 URL 重定向，通常在垃圾邮件中找到。
...
可以设置 ASF 选项，在、 关或到测试模式下，当您编辑内容筛选器策略。有关详细信息，请参阅[配置垃圾邮件筛选器策略](configure-your-spam-filter-policies.md)。测试模式下不可用的**NDR 退信**， **SPF 记录： 硬失败**，**条件发件人 ID 筛选： 硬失败**，和**批量邮件**选项。 
  
> > [!TIP]
> 可选信息可帮助用户是启用在测试模式下 ASF 选项以便最大限度地阻止垃圾邮件的更多成功考虑取决于您的环境。对于特定 ASF 选项的高垃圾邮件百分比的客户，我们建议您测试首先在生产环境中实现它们之前这些选项。> 建议关注网络钓鱼的组织打开**SPF 记录： 硬失败**选项。 
...
|**增加垃圾邮件得分部分** <br/> |启用后，这些选项的是匹配到 5 或 6，消息被认为是可疑的垃圾邮件设置垃圾邮件可信度 (SCL)。对邮件执行的操作将匹配内容筛选器策略中的**垃圾邮件**设置。<br/> ||
...
|**标记为垃圾邮件部分** <br/> |启用后，这些选项的匹配到 9，邮件被认为是某些垃圾邮件设置垃圾邮件可信度 (SCL)。对邮件执行的操作将匹配内容筛选器策略中的**高可信度垃圾邮件**设置。<br/> ||
...
> （内部部署和云托管的邮箱） 这两种情况，它还不是所需的服务，通过作为 Ndr 的合法弹跳消息将自动检测并传递给原始发件人发送的出站邮件启用此设置.> 测试模式下不可用于此选项。<br/><br/>> [!TIP]
> 提示： 有关退信消息和 EOP 的详细信息，请参阅[退信消息和 EOP](backscatter-messages-and-eop.md)。          |X-customspam： 退信消息 NDR<br/> | |批量邮件  <br/> |高级垃圾邮件筛选批量电子邮件的已停用并替换的批量和电子邮件的阈值设置。签出[垃圾邮件和批量邮件之间的区别是什么？](what-s-the-difference-between-junk-email-and-bulk-email.md)和[配置您的垃圾邮件筛选器策略](configure-your-spam-filter-policies.md)的详细信息以及如何配置设置 ings。<br/> ||
