...
您可以使用数据丢失防护 (DLP) 策略来识别、监视和保护跨 Office 365 的敏感信息。您希望组织中使用这些敏感信息的人员保持与您的 DLP 策略兼容, 但不希望在不必要的情况下阻止他们完成其工作。这是电子邮件通知和策略提示可帮助的地方。
...
|阻止访问，发送通知，并允许重写  <br/> |此项与组织中的策略相冲突。如果不解决此冲突, 则可能会阻止对此文件的访问。  <br/> |您的电子邮件与组织中的策略相冲突。邮件未传递给所有收件人。  <br/> |
...
- 只需通知此人该内容与 DLP 策略冲突, 以便他们可以采取措施来解决冲突。您可以使用默认文本 (请参阅下表) 或输入有关您的组织的特定策略的自定义文本。
    
- 允许用户替换 DLP 策略。（可选） 您可以：
    
  - 要求用户输入用于覆盖策略的业务理由。将记录此信息, 您可以在安全&amp;合规性中心的 "**报告**" 部分的 "DLP 报告" 中查看此信息。 
...
1. 第一条规则: 如果在文档中检测到的此敏感信息的实例数少于5个, 并且与组织内的人员共享文档, 则 "**发送通知**" 操作将显示策略提示。对于策略提示, 不需要覆盖选项, 因为此规则只是通知人员而不阻止访问。 
    
2. 第二个规则: 如果在文档中检测到此敏感信息的5个以上的实例, 并且与组织内的人员共享文档, 则 "**阻止对内容进行访问**" 操作将限制文件的权限, **** 通过提供业务理由, "发送通知" 操作可允许用户覆盖此规则中的操作。您的组织的业务有时需要内部人员共享 PII 数据, 并且您不希望您的 DLP 策略阻止此工作。 
    
3. 第三个规则: 如果在文档中检测到此敏感信息的5个以上的实例, 并且与组织外部的人员共享文档, 则 "**阻止对内容进行访问**" 操作将限制文件的权限, **发送通知**操作不允许用户覆盖此规则中的操作, 因为信息是在外部共享的。在任何情况下, 都不应允许组织中的人员共享组织外部的 PII 数据。 
    
以下几点有助于您对使用策略提示替换规则的理解：
  
- 替代的选项是 "按规则", 它将覆盖规则中的所有操作 (发送不能覆盖的通知除外)。
    
- 可以将内容与 DLP 策略中的多个规则相匹配, 但只会显示来自最严格的最高优先级规则的策略提示。例如, 来自阻止对内容的访问权限的规则中的策略提示将通过只发送通知的规则显示在策略提示上。这可防止用户看到策略级联的级联。
...
策略提示列出了内容的问题, 如果策略提示配置了这些选项, 您可以选择 "**解析**", 然后**覆盖**策略提示或**报告**误报。 
...
|阻止访问，发送通知，并允许重写  <br/> |此项与组织中的策略相冲突。如果不解决此冲突, 则可能会阻止对此文件的访问。  <br/> |
...
和策略提示也显示在 Backstage 视图 (在 "**文件**" 选项卡上)。 
  
![Backstage 在 Excel 2016 中显示策略提示](media/44c561f6-8f3f-4878-b1b0-b7543f8a4120.png)
  
如果 DLP 策略中的策略提示配置了这些选项, 您可以选择 "**解决**" 以**覆盖**策略提示或**报告**误报。 
  
![Excel 2016 Backstage 中的策略提示选项](media/5b3857ba-907e-456e-ae43-888b594c049c.png)
  
在每个 Office 2016 桌面程序中, 用户都可以选择关闭策略提示。如果关闭, 则简单通知的策略提示将不会显示在消息栏或 Backstage 视图 (在 "**文件**" 选项卡上) 中。但是, 有关阻止和替代的策略提示仍将显示, 并且仍将收到电子邮件通知。此外, 关闭策略提示不会将文档从已应用的任何 DLP 策略中排除。 
