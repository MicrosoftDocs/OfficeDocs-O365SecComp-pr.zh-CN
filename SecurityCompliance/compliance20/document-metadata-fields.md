...
---
# <a name="document-metadata-fields-in-advanced-ediscovery-preview"></a>高级电子数据展示中的文档元数据字段 (预览)

下表列出了高级电子数据展示 (预览) 中的工作集内的文档的元数据字段。 该表指示元数据字段的名称, 在工作集中运行查询时是否可以搜索该字段, 是否在查看工作集中的所选文档的文件元数据时是否存在该字段, 以及是否包含该字段导出文档。 

下表列出了高级电子数据展示 (预览) 中的工作集内的文档的元数据字段。 该表指示元数据字段的名称, 在工作集中运行查询时是否可以搜索该字段, 是否在查看工作集中的所选文档的文件元数据时是否存在该字段, 以及是否包含该字段导出文档。 

> [!NOTE]
> "**工作集**可搜索" 列中的括号中的值是您可以搜索的属性的名称。 "**文件元数据**" 列中的可查看括号中的值是您在查看文件元数据时显示的属性的名称。

|**字段名** </br>|**在工作集中可搜索** |**可在文件元数据中查看** |**Exported** |
|:-------------------------- |:---------------------------------------- |:------------------------|:------------------|
|Case 标记                  | 是 (标记)                                      |                         | 是         |
|合规性标签          |                                                 |                         | 是         |
|复合路径              |                                                 |                         | 是         |
...
|相关性分数 (问题)    | 是 (relevanceScore_issueNum)                   |                         | 是           |
|读取百分点值 (问题)    | 是 (readPercentile_issueNum)                   |                         | 是          |
|相关性标记 (问题)      | 是 (relevanceTag_issueNum)                     |                         | 是           |
|||||
