...
您可以使用 Office 365 中的内容搜索功能搜索和删除组织中所有邮箱的电子邮件。 这可以帮助您查找并删除可能有害的或存在高风险的电子邮件，如：
...
- 每个邮箱最多可以一次删除10个项目。 因为搜索和删除邮件的功能旨在作为事件响应工具, 所以此限制有助于确保快速从邮箱中删除邮件。 此功能并不是为了清理用户邮箱。 若要删除10个以上的项目, 您可以使用 Exchange Online PowerShell 中的**搜索-邮箱-DeleteContent**命令。 请参阅[搜索和删除邮件-管理员帮助](search-for-and-delete-messagesadmin-help.md)。
...
- **如何获取搜索和删除操作的状态？**
    ...
    运行 **Get-ComplianceSearchAction** 获取有关删除操作的状态。 请注意, 在运行**new-compliancesearchaction** cmdlet 时创建的对象是使用以下格式命名的: `<name of Content Search>_Purge`。 
