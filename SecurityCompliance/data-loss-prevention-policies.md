...
为了遵守业务标准和行业管理法规, 组织需要保护敏感信息, 并防止无意中泄露。您可能想要阻止泄露组织外部的敏感信息的示例包括财务数据或个人身份信息 (PII), 如信用卡号、社会保险号码或健康记录。使用 office 365 安全&amp;合规中心中的数据丢失防护 (DLP) 策略, 可以在 office 365 中识别、监视和自动保护敏感信息。
...
- **监视和保护桌面版本的 Excel 2016、PowerPoint 2016 和 Word 2016 中的敏感信息。**
    ...
    就像在 Exchange online、SharePoint online 和 OneDrive for business 中一样, 这些 Office 2016 桌面程序包含的功能可以识别敏感信息并应用 DLP 策略。当用户共享这些 Office 2016 程序中的内容时, DLP 提供连续监控。
    
- **帮助用户了解如何保持符合性，同时不会中断工作流。**
    
    您可以向用户介绍 DLP 策略, 并帮助他们保持合规性而不阻止其工作。例如, 如果用户尝试共享包含敏感信息的文档, 则 DLP 策略既可以向其发送电子邮件通知, 也可以在文档库的上下文中显示其策略提示, 以允许他们在拥有业务的情况下覆盖策略。合理化.同样的策略提示也会显示在 web 上的 outlook、outlook 2013 及更高版本、Excel 2016、PowerPoint 2016 和 Word 2016 中。
    
- **查看显示与组织的 DLP 策略匹配的内容的 DLP 报告。**
    
    若要评估您的组织如何遵守 DLP 策略, 您可以查看每个策略和规则的匹配时间有多少。如果 DLP 策略允许用户覆盖策略提示并报告误报, 则还可以查看用户报告的内容。
...
您可以使用规则满足特定的保护要求, 然后使用 DLP 策略将常见的保护要求组合在一起, 如符合特定法规所需的所有规则。
  
例如, 您可能有一个 DLP 策略, 可帮助您检测受健康保险便携性和责任法案 (HIPAA) 约束的信息的存在。此 DLP 策略可以通过查找包含与组织外部人员共享的这些敏感信息的任何文档来帮助保护所有 SharePoint Online 网站和所有 OneDrive for business 网站 (在 where) 上的 HIPAA 数据 (这些内容)。条件), 然后阻止对文档的访问和发送通知 (操作)。这些要求以各个规则的形式存储, 并作为 DLP 策略组合在一起, 以简化管理和报告。
...
规则是在组织内容中实施业务需求的。一个策略包含一个或多个规则, 并且每个规则都包含条件和操作。对于每个规则, 当满足条件时, 将自动执行这些操作。规则是按顺序执行的, 从每个策略中的最高优先级规则开始。
...
- 内容包含标签。有关详细信息, 请参阅下一节在[DLP 策略中使用标签作为条件](data-loss-prevention-policies.md#label)。
    
- 内容是与您组织的外部还是内部人员共享。
    
#### <a name="types-of-sensitive-information"></a>敏感信息类型

DLP 策略可帮助保护定义为**敏感信息类型**的敏感信息。Office 365 包括多个可供您使用的不同区域 (例如信用卡号、银行帐号、国家/地区 ID 和护照号码) 之间的多种常见敏感信息类型的定义。 
  
![可用敏感信息类型列表](media/3eaa9911-bc94-44be-902f-363dbf3b07fe.png)
  
当 DLP 策略查找敏感信息类型 (如信用卡号) 时, 它并不只是查找16位的数字。使用以下各项的组合定义和检测每种敏感信息类型:
...
- **限制对内容的访问**对于 "网站内容", 这意味着对文档的权限仅限于主网站集管理员、文档所有者和上次修改文档的人员之外的所有用户。这些人可以从文档中删除敏感信息或采取其他补救措施。当文档符合合规性时, 将自动还原原始权限。当阻止对文档的访问时, 该文档会在网站上的库中显示一个特殊的策略提示图标。 
...
您可以使用通知和替代来向用户介绍 DLP 策略, 并帮助他们保持合规性, 而不阻止其工作。例如, 如果用户尝试共享包含敏感信息的文档, 则 DLP 策略既可以向其发送电子邮件通知, 也可以在文档库的上下文中显示其策略提示, 以允许他们在拥有业务的情况下覆盖策略。合理化.
...
电子邮件通知和策略提示说明了内容与 DLP 策略冲突的原因。如果选择, 电子邮件通知和策略提示可允许用户通过报告误报或提供业务理由来覆盖规则。这有助于向用户介绍 DLP 策略并强制实施这些策略, 而不会阻止人员执行其工作。有关替代和误报的信息也会记录在报告中 (请参阅下面关于 DLP 报告的信息), 并将其包含在事件报告 (下一节) 中, 以便合规专员能够定期查看此信息。
...
- 仅来自最高优先级的策略提示将显示最受限制的规则。例如, 来自阻止对内容的访问权限的规则中的策略提示将通过只发送通知的规则显示在策略提示上。这可防止用户看到策略级联的级联。
...
"实例计数" 表示内容必须有多少个特定类型的敏感信息, 才能匹配该规则。例如, 如果在1和9之间标识了唯一的美国或英国护照号码, 则内容将与下面显示的规则匹配。
...
预配置的 DLP 策略模板可帮助您检测特定类型的敏感信息, 如 HIPAA 数据、PCI-DSS 数据、格雷姆-比利雷 Act 数据, 甚至是特定于区域设置的个人身份信息 (P.I.)。为了使您能够轻松地查找和保护常见类型的敏感信息, Office 365 中包含的策略模板已包含您开始时所需的最常见的敏感信息类型。
...
当您创建 DLP 策略时, 应考虑在完全强制实施之前, 将它们逐步滚动以评估其影响并测试其有效性。例如, 您不希望新的 DLP 策略无意中阻止用户需要访问的数千个文档的访问权限, 以便完成其工作。
  
如果您创建的 DLP 策略具有很大的潜在影响, 我们建议按照以下顺序进行操作:
  
1. **在测试模式下启动 (不带策略提示**), 然后使用 DLP 报告和任何事件报告来评估影响。您可以使用 DLP 报告查看策略匹配的数量、位置、类型和严重性。根据结果, 您可以根据需要对规则进行微调。在测试模式中, DLP 策略不会影响组织中工作人员的工作效率。 
    
2. **使用通知和策略提示移动到测试模式**, 以便您可以开始向用户讲授合规性策略, 并为要应用的规则做好准备。在此阶段, 您还可以要求用户报告误报, 以便您可以进一步优化规则。 
    
3. **对策略启动完全强制**, 以便应用规则中的操作和内容受保护。继续监视 DLP 报告和任何事件报告或通知, 以确保结果是您预期的。 
    
![使用测试模式和启用策略的选项](media/49fafaac-c6cb-41de-99c4-c43c3e380c3a.png)
  
您可以随时关闭 DLP 策略, 这会影响策略中的所有规则。但是, 也可以通过在规则编辑器中切换每条规则的状态来单独关闭这些规则。
  
![关闭策略中的规则的选项](media/f7b258ff-1b8b-4127-b580-83c6492f2bef.png)
  
## <a name="dlp-reports"></a>DLP 报告

创建并打开 DLP 策略后, 您需要验证它们是否按预期工作, 并帮助您保持合规性。使用 DLP 报告, 您可以快速查看一段时间内的 DLP 策略和规则匹配数, 以及误报和重写次数。对于每个报告, 可以按位置、时间范围筛选这些匹配, 甚至可以将其缩小到特定策略、规则或操作。
...
在所有 SharePoint Online 网站和 OneDrive for business 网站中, 文档会不断变化, 它们将不断地被创建、编辑、共享, 等等。这意味着文档可能会在任何时候与 DLP 策略发生冲突或符合要求。例如, 用户可以将不包含敏感信息的文档上传到其工作组网站, 但后来, 其他人可以编辑同一文档并向其添加敏感信息。
...
在 Outlook 中撰写邮件时, 如果要创建的内容是根据 DLP 策略进行评估, 则用户可以查看策略提示。在发送邮件之后, 它会将 DLP 策略作为邮件流的正常部分以及 exchange 管理中心内创建的 exchange 传输规则和 DLP 策略进行评估 (有关详细信息, 请参阅下一节)。DLP 策略同时扫描邮件和所有附件。
  
### <a name="policy-evaluation-in-the-office-2016-desktop-programs"></a>Office 2016 桌面程序中的策略评估

Excel 2016、PowerPoint 2016 和 Word 2016 包含标识敏感信息的相同功能, 并将 DLP 策略应用为 SharePoint Online 和 OneDrive for business。这些 Office 2016 程序直接从中央策略存储同步自己的 DLP 策略, 然后在用户处理从包含在 DLP 策略中的网站中打开的文档时, 根据 dlp 策略连续评估内容。
  
Office 2016 中的 DLP 策略评估设计为不会影响程序的性能或对内容进行处理的人员的工作效率。如果他们正在处理大型文档, 或者用户的计算机忙, 可能需要几秒钟的时间才能显示策略提示。
