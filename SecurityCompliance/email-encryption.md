...
Office 365 提供了多个加密选项, 可帮助您满足电子邮件安全的业务需求。本文介绍了在 Office 365 中对电子邮件进行加密的三种方法。若要了解有关 Office 365 中的所有安全功能的详细信息, 请访问[office 365 信任中心](http://go.microsoft.com/fwlink/p/?LinkID=282470)。本文介绍了 office 365 管理员可用于帮助保护 office 365 中的电子邮件的三种类型的加密:
...
加密是对信息进行编码的过程, 以便只有授权的收件人才能对信息进行解码和使用。Office 365 通过两种方式使用加密: 在服务中和作为客户控件。在该服务中, 默认情况下在 Office 365 中使用加密;您无需配置任何内容。例如, Office 365 使用传输层安全性 (TLS) 对两台服务器之间的连接或会话进行加密。 
...
|:-----|:-----|:-----|:-----|
|它是什么？|Office 365 邮件加密 (OME) 是一项基于 Azure 权限管理 (Azure RMS) 构建的服务，允许您向组织内外发送经加密的电子邮件，而无需考虑目标电子邮件地址（Gmail、Yahoo!Mail、Outlook.com 等）。  <br/> 作为管理员, 您可以设置用于定义加密条件的邮件流规则 (也称为传输规则)。当用户发送与某个规则匹配的邮件时, 系统会自动应用加密。  <br/> 若要查看加密邮件, 收件人可以获取一次性密码, 使用 Microsoft 帐户登录, 或使用与 Office 365 关联的工作或学校帐户进行登录。收件人还可以发送加密答复。他们不需要 Office 365 订阅即可查看加密邮件或发送加密答复。|IRM 是一种加密解决方案，还可向电子邮件应用用法限制。它可以帮助防止未经授权的人员打印、转发或复制敏感信息。  <br/> Office 365 中的 IRM 功能使用 Azure 权限管理 (Azure RMS)。 
|S/MIME 是一种基于证书的加密解决方案, 使您可以对邮件进行加密和数字签名。邮件加密有助于确保只有预期的收件人能够打开和阅读邮件。数字签名可帮助收件人验证发件人的身份。  <br/> 数字签名和邮件加密都可通过使用包含用于验证数字签名和加密或解密邮件的密钥的唯一数字证书来实现。  <br/> 若要使用 S/MIME, 您必须具有每个收件人的文件中的公钥。收件人必须维护自己的私钥, 这必须保持安全。如果收件人的私钥受到威胁, 收件人需要获取一个新的私钥, 并将公钥重新分发给所有潜在的发件人。|
|它会做什么？|OME：  <br/>  对发送到内部或外部收件人的邮件进行加密。  <br/>  允许用户将加密的邮件发送到任何电子邮件地址，包括 Outlook.com、Yahoo!Mail 和 Gmail。  <br/>  允许您以管理员身份自定义电子邮件查看门户, 以反映您的组织的品牌。  <br/>  Microsoft 安全管理和存储密钥, 因此您不必这样做。  <br/>  只要加密邮件（作为 HTML 附件发送）可以在浏览器中打开，就无需特殊的客户端软件。|IRM：  <br/>  使用加密和用法限制为电子邮件和附件提供联机和脱机保护。  <br/>  以管理员身份提供设置邮件流规则或 Outlook 保护规则的功能, 以自动应用 IRM 以选择邮件。  <br/>  允许用户在 outlook 或 web 上的 outlook 中手动应用模板 (以前称为 outlook web App)。|S/MIME 通过数字签名对发件人进行身份验证，并通过加密实现邮件保密性。|
|它不会做什么？|OME 不允许您对邮件应用使用限制。例如, 不能使用它阻止收件人转发或打印加密邮件。|某些应用程序可能不支持在所有设备上进行 IRM 电子邮件。有关支持 IRM 电子邮件的这些产品和其他产品的详细信息, 请参阅[客户端设备功能](https://technet.microsoft.com/library/dn655136.aspx#BKMK_ClientCapabilities)。|S/MIME 不允许扫描加密邮件中的恶意软件、垃圾邮件或策略。|
...
有关 Office 365 订阅的电子邮件加密选项的信息, 请参阅[Exchange Online 服务说明](https://technet.microsoft.com/en-us/library/exchange-online-service-description.aspx)。在这里, 您可以找到有关以下加密功能的信息:
...
"静止的数据" 是指在传输中不活动的数据。在 Office 365 中, 使用 BitLocker 驱动器加密对 rest 上的电子邮件数据进行加密。BitLocker 将对 Office 365 数据中心中的硬盘驱动器进行加密, 以提供增强保护以防未经授权的访问。若要了解详细信息, 请参阅[BitLocker 概述](https://go.microsoft.com/fwlink/p/?LinkId=394737)。
