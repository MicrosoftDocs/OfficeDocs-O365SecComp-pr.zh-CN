...
监视组织中发送或接收的电子邮件是否带有攻击性的语言。 模型使用机器学习、人工智能和关键字的组合来识别不适当的电子邮件, 作为反骚扰和网络威胁监视要求的一部分。 若要阻止或阻止攻击性语言用于组织中的其他通信, 请创建一个使用冒犯性术语的[自定义关键字词典](create-a-keyword-dictionary.md)的[数据丢失防护策略](create-test-tune-dlp-policy.md)。
