...
使用下面的脚本和 .csv 文件，可批量创建和发布标签及标签策略。首先，在 Excel 中创建标签列表和标签策略列表。然后，使用 PowerShell 批量创建这些列表中的标签和标签策略。这样就可以更轻松地一次性创建和发布保留计划所需的全部标签。
...
## <a name="step-1-create-a-csv-file-for-creating-the-labels"></a>第 1 步：创建用于创建标签的 .csv 文件

首先，创建包含标签列表及其保留设置的 .csv 文件。可将下面的示例用作模板，具体方法是将示例复制到 Excel 中，将文本转换为列（在 Excel 中依次单击“数据”**** 选项卡 \>“将文本转换为列”****\>“分隔符号”****\>“逗号”****\>“常规”****），再将工作表另存为 .csv 文件，并保存到易于查找的位置。
  
若要详细了解此 cmdlet 的参数值，请参阅 [New-ComplianceTag](https://go.microsoft.com/fwlink/?linkid=866511)。
  
注意：
  
- 如果你没有提供用于创建标签的源文件，脚本会继续运行，并提示你提供用于发布标签的源文件（请参阅下一部分），否则脚本只会发布现有标签。
    
- 如果 .csv 文件中有标签与现有标签同名，脚本会跳过创建此标签。原则是不创建重复标签。
...
接下来，创建包含标签列表及其位置和其他设置的 .csv 文件。可将下面的示例用作模板，具体方法是将示例复制到 Excel 中，将文本转换为列（在 Excel 中依次单击“数据”**** 选项卡 \>“将文本转换为列”****\>“分隔符号”****\>“逗号”****\>“常规”****），再将工作表另存为 .csv 文件，并保存到易于查找的位置。
  
若要详细了解此 cmdlet 的参数值，请参阅 [New-RetentionCompliancePolicy](https://go.microsoft.com/fwlink/?linkid=866512)。
  
注意：
  
- 如果你没有提供用于发布标签的源文件，脚本会创建标签（请参阅上一部分），但不会发布标签。
    
- 如果 .csv 文件中有标签策略与现有标签策略同名，脚本会跳过创建此标签策略。原则是不创建重复标签策略。
    
- 脚本仅发布手动应用于内容的标签。此脚本不支持自动应用于内容的标签。
...
## <a name="step-5-run-the-powershell-script-to-create-and-publish-the-labels"></a>第 5 步：运行 PowerShell 脚本以创建并发布标签

连接到安全与合规中心 PowerShell 后，下一步是运行用于创建和发布标签的脚本。
...
运行后的脚本生成一个日志文件，用于记录它执行的所有操作以及操作是否成功。日志文件包含已创建标签和已发布标签的所有元数据。日志文件位于下面的位置，但请注意，文件名中的数字会有所不同。
