...
## <a name="checking-for-and-removing-restrictions-using-powershell"></a>使用 PowerShell 检查和删除限制
受限制的用户的 PowerShell 命令为:
- `Get-BlockedSenderAddress`: 运行以检索受限制为发送电子邮件的用户的列表
- `Remove-BlockedSenderAddress`: 运行以删除受限制的用户
