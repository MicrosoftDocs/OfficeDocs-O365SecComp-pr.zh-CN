...
1. <span data-ttu-id="e68b2-111">转到 [https://servicetrust.microsoft.com](https://servicetrust.microsoft.com/)。</span><span class="sxs-lookup"><span data-stu-id="e68b2-111">Go to [https://servicetrust.microsoft.com](https://servicetrust.microsoft.com/).</span></span>
...
7. <span data-ttu-id="e68b2-142">若要从此角色中删除用户, 请选择 "用户", 然后选择 "**删除**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-142">To remove users from this role, select the users and select **Delete**.</span></span>
...
<span data-ttu-id="e68b2-159">使用组时, 请记住:</span><span class="sxs-lookup"><span data-stu-id="e68b2-159">When you work with groups, remember:</span></span>
...
- <span data-ttu-id="e68b2-176">**维:** 查看、添加和自定义模板、评估和操作项的元数据, 以允许您为筛选器创建自定义透视。</span><span class="sxs-lookup"><span data-stu-id="e68b2-176">**Dimensions:** View, add and customize metadata for Templates, Assessments, and Action Items that allow you to create custom pivots for filters.</span></span>
...
#### <a name="edit-a-dimension"></a><span data-ttu-id="e68b2-195">编辑维度</span><span class="sxs-lookup"><span data-stu-id="e68b2-195">Edit a Dimension</span></span>
...
1. <span data-ttu-id="e68b2-212">打开 "**租户管理**" 并选择 "**所有者**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-212">Open **Tenant Management** and select **Owners**.</span></span>
...
1. <span data-ttu-id="e68b2-237">选择 " **+ 添加客户操作**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-237">Select **+ Add Customer Action**.</span></span>
...
1. <span data-ttu-id="e68b2-255">在 "评估" 仪表板中, 选择 " **+ 添加评估**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-255">In the Assessments dashboard, select **+ Add Assessment**.</span></span>
...
### <a name="viewing-assessments"></a><span data-ttu-id="e68b2-290">查看评估</span><span class="sxs-lookup"><span data-stu-id="e68b2-290">Viewing Assessments</span></span>
...
- <span data-ttu-id="e68b2-335">控件的详细信息, 按控件系列分组, 并提供指向客户操作和 Microsoft 实现详细信息的链接:</span><span class="sxs-lookup"><span data-stu-id="e68b2-335">Details of the controls, grouped by Control Family, with links to customer actions and Microsoft implementation details:</span></span>
    ...
    - <span data-ttu-id="e68b2-339">**Microsoft 操作**显示 microsoft 内部框架中应用于所选证书控制的控件列表。</span><span class="sxs-lookup"><span data-stu-id="e68b2-339">**Microsoft Actions** displays the list of controls from Microsoft’s internal framework that apply to the selected certification control.</span></span> <span data-ttu-id="e68b2-340">对于每个内部控件, 选择 "已**实施**" 以查看 Microsoft 的实施和测试详细信息, 以及测试结果和测试日期, 如下所示。</span><span class="sxs-lookup"><span data-stu-id="e68b2-340">For each internal control, select **Implemented** to see Microsoft’s implementation and test details, along with the test result and test date, as shown below.</span></span>
...
### <a name="export-an-assessment-report"></a><span data-ttu-id="e68b2-347">导出评估报告</span><span class="sxs-lookup"><span data-stu-id="e68b2-347">Export an Assessment report</span></span>
...
#### <a name="archive-an-assessment"></a><span data-ttu-id="e68b2-363">存档评估</span><span class="sxs-lookup"><span data-stu-id="e68b2-363">Archive an Assessment</span></span>
...
### <a name="controls"></a><span data-ttu-id="e68b2-382">控件</span><span class="sxs-lookup"><span data-stu-id="e68b2-382">Controls</span></span>
...
#### <a name="reassign-action-items"></a><span data-ttu-id="e68b2-411">重新分配操作项</span><span class="sxs-lookup"><span data-stu-id="e68b2-411">Reassign Action Items</span></span>
...
<span data-ttu-id="e68b2-423">仪表板将显示每个模板以及与模板关联的证书和产品、创建模板的日期和上次修改的日期、客户和 Microsoft 托管控件的数量以及最大符合性分数模板, 以及模板的状态 (例如, "已批准"、"待定审批"、"已导入")。</span><span class="sxs-lookup"><span data-stu-id="e68b2-423">The dashboard displays each Template, along with the Certification and Product associated with the Template, the dates on which the Template was created and last modified, the number of customer and Microsoft-managed controls, the maximum Compliance Score for the Template, and the status of the Template (for example, Approved, Pending Approval, Imported).</span></span>
...
#### <a name="controlfamily-tab"></a><span data-ttu-id="e68b2-439">ControlFamily 选项卡</span><span class="sxs-lookup"><span data-stu-id="e68b2-439">ControlFamily tab</span></span>
...
<span data-ttu-id="e68b2-454">此选项卡包含定义各个操作的列, 并且包含操作标题、所有权和维度等详细信息。</span><span class="sxs-lookup"><span data-stu-id="e68b2-454">This tab includes columns that define individual Actions, and it includes details like action title, ownership, and dimensions.</span></span> <span data-ttu-id="e68b2-455">此选项卡的列必须按下面列出的顺序在 Excel 中进行排序, 如下所示:</span><span class="sxs-lookup"><span data-stu-id="e68b2-455">The columns for this tab, which must be ordered within Excel in the order listed below, are:</span></span> 

- <span data-ttu-id="e68b2-456">**actionTitle:** 操作的标题。</span><span class="sxs-lookup"><span data-stu-id="e68b2-456">**actionTitle:** Title of the action.</span></span> <span data-ttu-id="e68b2-457">每个标题都必须是唯一的, 我们建议使用 Pascal 大小写。</span><span class="sxs-lookup"><span data-stu-id="e68b2-457">Each title must be unique, and we recommend using Pascal case.</span></span>
- <span data-ttu-id="e68b2-458">**actionRelatedODVs:** 以分号分隔的 actionTitles 列表, 它们是 actionTitle 列中列出的子元素的父项。</span><span class="sxs-lookup"><span data-stu-id="e68b2-458">**actionRelatedODVs:** Double semicolon-delimited list of actionTitles that are parents of the child listed in the actionTitle column.</span></span> <span data-ttu-id="e68b2-459">在父/子关系中, 父项代表高水位线。</span><span class="sxs-lookup"><span data-stu-id="e68b2-459">In a parent/child relationship, the parent represents the high watermark.</span></span> <span data-ttu-id="e68b2-460">因此, 如果完成父操作, 还将完成所有子操作。</span><span class="sxs-lookup"><span data-stu-id="e68b2-460">Thus, if you complete a parent action, you also complete all child actions.</span></span> <span data-ttu-id="e68b2-461">例如, 如果您具有类似的要求, 但具有不同的标准定义值 (如密码长度), 其中一个标准/法规至少需要15个字符, 而另一个标准/要求至少需要12个或10个。</span><span class="sxs-lookup"><span data-stu-id="e68b2-461">For example, when you have similar requirements but different standard-defined values, such as password length, where one standard/regulation requires a minimum of 15 characters, and another requires a minimum of 12 or 10.</span></span> <span data-ttu-id="e68b2-462">15是此示例中的父级, 如果至少配置15个字符, 则还应满足在其他评估中建议的12个或10个字符的操作。</span><span class="sxs-lookup"><span data-stu-id="e68b2-462">15 would be the parent in this example, and if you configure a minimum of 15 characters, you also satisfy the actions that recommend 12 or 10 characters in other assessments.</span></span>

    > [!NOTE]
    > <span data-ttu-id="e68b2-463">ActionRelatedODVs 列是架构的必需列;但是, 功能 (相关操作) 在合规性管理器 (预览) 中不可用。</span><span class="sxs-lookup"><span data-stu-id="e68b2-463">The actionRelatedODVs column is a required column for the schema; however, the feature (related actions) is not available in Compliance Manager (Preview).</span></span>  <span data-ttu-id="e68b2-464">计划在更高版本中添加它。</span><span class="sxs-lookup"><span data-stu-id="e68b2-464">It is scheduled to be added in a later release.</span></span>

- <span data-ttu-id="e68b2-465">**actionDimensionValues:** 使用以下格式的 "维" 选项卡中的适用维度的两个以分号分隔的列表:</span><span class="sxs-lookup"><span data-stu-id="e68b2-465">**actionDimensionValues:** Double semicolon-delimited list of applicable dimensions from the Dimensions tab, using the following format:</span></span>

    ```
    Dimension Key::Dimension Value;;Dimension Key::Dimension Value.
    ```
    
    <span data-ttu-id="e68b2-466">例如：</span><span class="sxs-lookup"><span data-stu-id="e68b2-466">For example:</span></span>

    ```
    Product::Office 365;;Certification::NIST CSF
    ```

    <span data-ttu-id="e68b2-467">自定义模板中使用的所有维度都必须在导入文件的 "维度" 选项卡上列出, 即使它们已列在 "维度" 仪表板上也是如此。</span><span class="sxs-lookup"><span data-stu-id="e68b2-467">All Dimensions that are used in a custom Template must be listed on the Dimensions tab of the import file, even if they are already listed on the Dimensions dashboard.</span></span> <span data-ttu-id="e68b2-468">如果要添加新的维度键或值, 则必须首先将其添加到 "维度" 仪表板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-468">If you are adding new Dimension Keys or Values, you must add them first to the Dimensions dashboard.</span></span>
- <span data-ttu-id="e68b2-469">**actionScore:** 每个操作的数字值, 表示该操作的分数。</span><span class="sxs-lookup"><span data-stu-id="e68b2-469">**actionScore:** Numeric value for each Action, which represents the score for that action.</span></span> <span data-ttu-id="e68b2-470">我们建议遵循内置评估使用的记分模型, 这取决于每个操作的用途和实施。</span><span class="sxs-lookup"><span data-stu-id="e68b2-470">We recommend following the scoring model used by the built-in assessments, which is based on each Action’s purpose and enforcement.</span></span>
- <span data-ttu-id="e68b2-471">**actionOwnership:** 以分号分隔的所有者列表。</span><span class="sxs-lookup"><span data-stu-id="e68b2-471">**actionOwnership:** Double semicolon-delimited list of Owners.</span></span> <span data-ttu-id="e68b2-472">所有列出的所有者都必须包含在 "所有权" 选项卡上。</span><span class="sxs-lookup"><span data-stu-id="e68b2-472">All listed Owners must be included on the Ownership tab.</span></span>
- <span data-ttu-id="e68b2-473">**actionDescription:** 每个操作的文本, 必须是唯一的。</span><span class="sxs-lookup"><span data-stu-id="e68b2-473">**actionDescription:** Text of each Action, which must be unique.</span></span> <span data-ttu-id="e68b2-474">此字段支持 Markdown 语言, 如下所述。</span><span class="sxs-lookup"><span data-stu-id="e68b2-474">This field supports Markdown Language as described below.</span></span>

#### <a name="ownership-tab"></a><span data-ttu-id="e68b2-475">所有权选项卡</span><span class="sxs-lookup"><span data-stu-id="e68b2-475">Ownership tab</span></span>

<span data-ttu-id="e68b2-476">此选项卡包含用于定义每个操作的所有者的列。</span><span class="sxs-lookup"><span data-stu-id="e68b2-476">This tab includes columns that define owners for each action.</span></span>  <span data-ttu-id="e68b2-477">此选项卡的列必须按下面列出的顺序在 Excel 中进行排序, 如下所示:</span><span class="sxs-lookup"><span data-stu-id="e68b2-477">The columns for this tab, which must be ordered within Excel in the order listed below, are:</span></span>

- <span data-ttu-id="e68b2-478">**ownershipName:** 所有者/责任方的唯一名称。</span><span class="sxs-lookup"><span data-stu-id="e68b2-478">**ownershipName:** Unique name of owner/responsible party.</span></span>
- <span data-ttu-id="e68b2-479">**ownershipDescription:** 所有者/责任方的说明。</span><span class="sxs-lookup"><span data-stu-id="e68b2-479">**ownershipDescription:** Description of the owner/responsible party.</span></span>

#### <a name="dimensions-tab"></a><span data-ttu-id="e68b2-480">维度选项卡</span><span class="sxs-lookup"><span data-stu-id="e68b2-480">Dimensions tab</span></span>

<span data-ttu-id="e68b2-481">此选项卡包含定义可与操作相关联的维度的列。</span><span class="sxs-lookup"><span data-stu-id="e68b2-481">This tab includes columns that define the Dimensions that can be associated with an Action.</span></span>  <span data-ttu-id="e68b2-482">此选项卡的列必须按下面列出的顺序在 Excel 中进行排序, 如下所示:</span><span class="sxs-lookup"><span data-stu-id="e68b2-482">The columns for this tab, which must be ordered within Excel in the order listed below, are:</span></span>

- <span data-ttu-id="e68b2-483">**dimensionKey:** 用于维度的键的列表。</span><span class="sxs-lookup"><span data-stu-id="e68b2-483">**dimensionKey:** List of Keys used for Dimensions.</span></span> <span data-ttu-id="e68b2-484">例如, 产品、认证等。</span><span class="sxs-lookup"><span data-stu-id="e68b2-484">For example, Product, Certification, etc.</span></span>
- <span data-ttu-id="e68b2-485">**dimensionValue:** 每个维度键的唯一值。</span><span class="sxs-lookup"><span data-stu-id="e68b2-485">**dimensionValue:** Unique value for each dimension key.</span></span> <span data-ttu-id="e68b2-486">例如, Office 365、Intune、Azure、自定义产品等。</span><span class="sxs-lookup"><span data-stu-id="e68b2-486">For example, Office 365, Intune, Azure, Custom Product, etc.</span></span>
- <span data-ttu-id="e68b2-487">**allowMultiSelect:** 使用 TRUE 或 FALSE 可指示可以为单个维度键选择多个维度值。</span><span class="sxs-lookup"><span data-stu-id="e68b2-487">**allowMultiSelect:** Use TRUE or FALSE to indicate that multiple dimension values can be selected for a single dimension key.</span></span>

#### <a name="using-markdown-language-in-description-fields"></a><span data-ttu-id="e68b2-488">在 "说明" 字段中使用 Markdown 语言</span><span class="sxs-lookup"><span data-stu-id="e68b2-488">Using Markdown Language in Description Fields</span></span>

<span data-ttu-id="e68b2-489">模板和评估支持对某些文本元素和格式使用 Markdown 语言。</span><span class="sxs-lookup"><span data-stu-id="e68b2-489">Templates and Assessments support the use of Markdown language for some text elements and formatting.</span></span>  <span data-ttu-id="e68b2-490">在合规性管理器中使用的 Markdown 语言有三个格式元素:</span><span class="sxs-lookup"><span data-stu-id="e68b2-490">There are three formatting elements of Markdown language that are used in Compliance Manager:</span></span>

- <span data-ttu-id="e68b2-491">项目符号和编号列表</span><span class="sxs-lookup"><span data-stu-id="e68b2-491">Bullets and Numbered lists</span></span>
- <span data-ttu-id="e68b2-492">Hyperlinks</span><span class="sxs-lookup"><span data-stu-id="e68b2-492">Hyperlinks</span></span>
- <span data-ttu-id="e68b2-493">粗体</span><span class="sxs-lookup"><span data-stu-id="e68b2-493">Boldface</span></span>

<span data-ttu-id="e68b2-494">项目符号表示为星号而不是 Word 或 Excel 项目符号。</span><span class="sxs-lookup"><span data-stu-id="e68b2-494">Bullets are represented as asterisks instead of Word or Excel bullets.</span></span> <span data-ttu-id="e68b2-495">例如：</span><span class="sxs-lookup"><span data-stu-id="e68b2-495">For example:</span></span>
...
<span data-ttu-id="e68b2-496">数字表示为数字, 但在缩进 (每个级别三个空格) 和仅用于所有子级别的数字 (例如, 不带字母) 之间使用空格。</span><span class="sxs-lookup"><span data-stu-id="e68b2-496">Numbers are represented as numbers, but with spaces for indentation (three spaces per level) and only numbers used for all sublevels (for example, no letters).</span></span>  <span data-ttu-id="e68b2-497">例如：</span><span class="sxs-lookup"><span data-stu-id="e68b2-497">For example:</span></span>
   1. <span data-ttu-id="e68b2-498">Item A</span><span class="sxs-lookup"><span data-stu-id="e68b2-498">Item A</span></span>
   2. <span data-ttu-id="e68b2-499">项目 B</span><span class="sxs-lookup"><span data-stu-id="e68b2-499">Item B</span></span>
      1. <span data-ttu-id="e68b2-500">子项目 A</span><span class="sxs-lookup"><span data-stu-id="e68b2-500">Sub-item A</span></span>
      2. <span data-ttu-id="e68b2-501">子项目 B</span><span class="sxs-lookup"><span data-stu-id="e68b2-501">Sub-item B</span></span>
   3. <span data-ttu-id="e68b2-502">项目 C</span><span class="sxs-lookup"><span data-stu-id="e68b2-502">Item C</span></span>
   4. <span data-ttu-id="e68b2-503">项目 D</span><span class="sxs-lookup"><span data-stu-id="e68b2-503">Item D</span></span>
      1. <span data-ttu-id="e68b2-504">子项目 A</span><span class="sxs-lookup"><span data-stu-id="e68b2-504">Sub-item A</span></span>
      2. <span data-ttu-id="e68b2-505">子项目 B</span><span class="sxs-lookup"><span data-stu-id="e68b2-505">Sub-item B</span></span>
   5. <span data-ttu-id="e68b2-506">Item E</span><span class="sxs-lookup"><span data-stu-id="e68b2-506">Item E</span></span>

<span data-ttu-id="e68b2-507">通过在超链接文本周围加上括号并将超链接本身放在右方括号旁边的括号中来构造超链接。</span><span class="sxs-lookup"><span data-stu-id="e68b2-507">Hyperlinks are constructed by placing brackets around the hyperlink text and the hyperlink itself in parentheses immediately next to the close bracket.</span></span>  <span data-ttu-id="e68b2-508">例如：</span><span class="sxs-lookup"><span data-stu-id="e68b2-508">For example:</span></span>

```
Click [here](https://www.microsoft.com) to go to Microsoft’s home page.
```
<span data-ttu-id="e68b2-509">此文本呈现如下: 单击[此处](https://www.microsoft.com)转到 Microsoft 主页。</span><span class="sxs-lookup"><span data-stu-id="e68b2-509">This text renders as follows:  Click [here](https://www.microsoft.com) to go to Microsoft’s home page.</span></span>
<span data-ttu-id="e68b2-510">如上面的示例中所示, 合规性管理器不呈现带下划线的 Url。</span><span class="sxs-lookup"><span data-stu-id="e68b2-510">As shown in the above example, Compliance Manager does not render URLs with underlining.</span></span>

<span data-ttu-id="e68b2-511">粗体文本只是文本的每一侧都加粗的两个星号。</span><span class="sxs-lookup"><span data-stu-id="e68b2-511">Boldface text is just two asterisks on each side of the text to be bolded.</span></span>  <span data-ttu-id="e68b2-512">例如：</span><span class="sxs-lookup"><span data-stu-id="e68b2-512">For example:</span></span>

```
**This text will render in bold**
```
<span data-ttu-id="e68b2-513">**此文本以粗体呈现**</span><span class="sxs-lookup"><span data-stu-id="e68b2-513">**This text renders in bold**</span></span>

### <a name="create-a-template"></a><span data-ttu-id="e68b2-514">创建模板</span><span class="sxs-lookup"><span data-stu-id="e68b2-514">Create a Template</span></span>

<span data-ttu-id="e68b2-515">您可以通过复制现有模板或从 Excel 导入模板数据来创建模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-515">You can create a Template by copying an existing Template or by importing Template data from Excel.</span></span> <span data-ttu-id="e68b2-516">从 Excel 导入数据时, 模板需要两个不同的合规性管理器管理员发布数据 (一个发布, 另一个用于审批)。</span><span class="sxs-lookup"><span data-stu-id="e68b2-516">When importing data from Excel, the Template requires two different Compliance Manager Administrators to publish the data (one to publish, and one to approve).</span></span>

#### <a name="create-a-template-by-copying-an-existing-template"></a><span data-ttu-id="e68b2-517">通过复制现有模板创建模板</span><span class="sxs-lookup"><span data-stu-id="e68b2-517">Create a Template by copying an existing Template</span></span>

1. <span data-ttu-id="e68b2-518">打开 "**模板**" 仪表板, 然后选择 " **+ 添加模板**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-518">Open the **Templates** dashboard and select **+ Add Template**.</span></span>
2. <span data-ttu-id="e68b2-519">在 "**输入模板名称**" 字段中, 为模板提供一个唯一的名称。</span><span class="sxs-lookup"><span data-stu-id="e68b2-519">In the **Enter template name** field, provide a unique name for the Template.</span></span>
3. <span data-ttu-id="e68b2-520">选中 "**从现有模板复制**" 复选框, 然后从下拉列表中选择要复制的模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-520">Check the **Copy from an existing template** checkbox and select the template you want to copy from the dropdown.</span></span>
4. <span data-ttu-id="e68b2-521">(可选) 添加任何其他维度。</span><span class="sxs-lookup"><span data-stu-id="e68b2-521">Optionally add any additional Dimensions.</span></span>
5. <span data-ttu-id="e68b2-522">选择 "**添加到仪表板**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-522">Select **Add to Dashboard**.</span></span>

#### <a name="create-a-template-by-importing-data"></a><span data-ttu-id="e68b2-523">通过导入数据创建模板</span><span class="sxs-lookup"><span data-stu-id="e68b2-523">Create a Template by importing data</span></span>

1. <span data-ttu-id="e68b2-524">打开 "**模板**" 仪表板, 然后选择 " **+ 添加模板**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-524">Open the **Templates** dashboard and select **+ Add Template**.</span></span>
2. <span data-ttu-id="e68b2-525">在 "**输入模板名称**" 字段中, 为模板提供一个唯一的名称。</span><span class="sxs-lookup"><span data-stu-id="e68b2-525">In the **Enter template name** field, provide a unique name for the Template.</span></span>
3. <span data-ttu-id="e68b2-526">添加一个或多个维度。</span><span class="sxs-lookup"><span data-stu-id="e68b2-526">Add one or more Dimensions.</span></span> <span data-ttu-id="e68b2-527">即使您使用的尺寸已列在维度仪表板中, 它们仍必须在导入文件中列出。</span><span class="sxs-lookup"><span data-stu-id="e68b2-527">Even if the Dimensions you use are already listed on the Dimensions dashboard, they must still be listed in the import file.</span></span>
4. <span data-ttu-id="e68b2-528">选择 "**浏览**" 导航到导入文件的位置, 选择该文件, 然后选择 "**打开**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-528">Select **Browse** to navigate to the location of the import file, select it, and select **Open**.</span></span>
5. <span data-ttu-id="e68b2-529">将验证导入文件, 并指示检测到的控件和控件系列的数量。</span><span class="sxs-lookup"><span data-stu-id="e68b2-529">The import file will be validated and indicate the number of controls and control families that were detected.</span></span> <span data-ttu-id="e68b2-530">如果有错误, 则会向包含错误详细信息的导入文件的修改版本提供链接。</span><span class="sxs-lookup"><span data-stu-id="e68b2-530">If there are errors, a link will be provided to a modified version of the import file that includes error details.</span></span> <span data-ttu-id="e68b2-531">必须先解决所有错误, 然后才会导入数据。</span><span class="sxs-lookup"><span data-stu-id="e68b2-531">All errors must be resolved before the data will be imported.</span></span>
6. <span data-ttu-id="e68b2-532">数据通过验证后, 选择 "**添加到仪表板**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-532">Once the data passes validation, select **Add to Dashboard**.</span></span>
7. <span data-ttu-id="e68b2-533">导入的模板将显示在 "**模板**" 仪表板上, 其状态为 "已**导入**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-533">The imported Template appears on the **Templates** dashboard and it has a status of **Imported**.</span></span> <span data-ttu-id="e68b2-534">选择省略号 (...), 然后选择 "**发布**" 以发布模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-534">Select the ellipses (…) and select **Publish** to publish the Template.</span></span> <span data-ttu-id="e68b2-535">当出现确认消息时, 请选择 "**发布**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-535">When the confirmation message appears, select **Publish**.</span></span> <span data-ttu-id="e68b2-536">模板状态将更改为 "**待审批**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-536">The Template status changes to **Pending Approval**.</span></span>
8. <span data-ttu-id="e68b2-537">另一个具有合规性管理器管理员角色的用户必须批准模板仪表板中的模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-537">Another user with the Compliance Manager Administrator role must approve the Template in the Templates dashboard.</span></span> <span data-ttu-id="e68b2-538">他们必须选择省略号 (...), 然后选择 "**批准**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-538">They must select the ellipses (…) and select **Approve**.</span></span> <span data-ttu-id="e68b2-539">当出现确认消息时, 选择 "**批准**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-539">When the confirmation message appears, select **Approve**.</span></span> <span data-ttu-id="e68b2-540">现在即可使用该模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-540">The Template is now ready for use.</span></span>

### <a name="customize-a-template"></a><span data-ttu-id="e68b2-541">自定义模板</span><span class="sxs-lookup"><span data-stu-id="e68b2-541">Customize a Template</span></span>

<span data-ttu-id="e68b2-542">可以通过其他自定义控件自定义模板。</span><span class="sxs-lookup"><span data-stu-id="e68b2-542">Templates can be customized through the additional of custom controls.</span></span> <span data-ttu-id="e68b2-543">所有自定义控件都被视为客户托管的控件。</span><span class="sxs-lookup"><span data-stu-id="e68b2-543">All custom controls are considered customer-managed Controls.</span></span>

#### <a name="add-a-custom-control-to-a-template"></a><span data-ttu-id="e68b2-544">向模板中添加自定义控件</span><span class="sxs-lookup"><span data-stu-id="e68b2-544">Add a custom control to a Template</span></span>

1. <span data-ttu-id="e68b2-545">打开要修改的**模板**。</span><span class="sxs-lookup"><span data-stu-id="e68b2-545">Open the **Template** you want to modify.</span></span>
2. <span data-ttu-id="e68b2-546">选择 " **+ 添加**自定义控件"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-546">Select **+ Add** custom control.</span></span>
3. <span data-ttu-id="e68b2-547">从下拉列表中选择一个**控制族**, 或输入一个新的控制族 (如果不存在)。</span><span class="sxs-lookup"><span data-stu-id="e68b2-547">Select a **Control Family** from the dropdown or enter a new control family if it does not exist.</span></span>
4. <span data-ttu-id="e68b2-548">为 "**控件 ID** " 字段中的控件提供一个唯一的名称或 ID。</span><span class="sxs-lookup"><span data-stu-id="e68b2-548">Provide a unique name or ID for the control in the **Control ID** field.</span></span>
5. <span data-ttu-id="e68b2-549">在 "**标题**" 字段中提供控件标题。</span><span class="sxs-lookup"><span data-stu-id="e68b2-549">Provide the control title in the **Title** field.</span></span>
6. <span data-ttu-id="e68b2-550">提供 "**说明**" 字段中控件的要求和其他信息。</span><span class="sxs-lookup"><span data-stu-id="e68b2-550">Provide the requirements and other information for the control in the **Description** field.</span></span>
7. <span data-ttu-id="e68b2-551">选择 "**分配客户**操作"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-551">Select **Assign Customer** Action.</span></span>
8. <span data-ttu-id="e68b2-552">找到要分配给控件的操作:</span><span class="sxs-lookup"><span data-stu-id="e68b2-552">Locate the Action(s) you want to assign to the control:</span></span>
    - <span data-ttu-id="e68b2-553">使用 "**按维度筛选**" 以使用分配给操作的维度来查找和列出它们。</span><span class="sxs-lookup"><span data-stu-id="e68b2-553">Use **Filter by Dimension** to use dimensions assigned to the Action(s) to locate and list them.</span></span>
    - <span data-ttu-id="e68b2-554">使用 "**按所有者筛选**" 以使用分配给操作的所有者来查找和列出它们。</span><span class="sxs-lookup"><span data-stu-id="e68b2-554">Use **Filter by Owner** to use the owner(s) assigned to the Action(s) to locate and list them.</span></span>
    - <span data-ttu-id="e68b2-555">从下拉列表中选择**操作类型**以按类型列出操作。</span><span class="sxs-lookup"><span data-stu-id="e68b2-555">Select an **Action Type** from the dropdown to list Actions by type.</span></span>
    - <span data-ttu-id="e68b2-556">输入要查找的操作的标题并将其列出。</span><span class="sxs-lookup"><span data-stu-id="e68b2-556">Enter the title of the Action to locate and list it.</span></span>
9. <span data-ttu-id="e68b2-557">使用步骤8中的条件, 将显示**匹配操作**的列表。</span><span class="sxs-lookup"><span data-stu-id="e68b2-557">Using the criteria in Step 8, a list of **Matching Action(s)** will appear.</span></span> <span data-ttu-id="e68b2-558">选择要分配给控件的第一个操作。</span><span class="sxs-lookup"><span data-stu-id="e68b2-558">Select the first Action you want to assign to the control.</span></span>
10. <span data-ttu-id="e68b2-559">将显示操作的详细信息。</span><span class="sxs-lookup"><span data-stu-id="e68b2-559">The details of the Action appear.</span></span> <span data-ttu-id="e68b2-560">选择要使用的**说明**, 然后选择 "**完成**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-560">Select the **Description** you want to use and select **Done**.</span></span>
11. <span data-ttu-id="e68b2-561">对每个要分配的其他操作重复步骤9和10。</span><span class="sxs-lookup"><span data-stu-id="e68b2-561">Repeat Steps 9 and 10 for each additional Action you want to assign.</span></span>
12. <span data-ttu-id="e68b2-562">选择所有适用的操作后, 选择 "**分配**"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-562">When all applicable Actions have been selected, select **Assign**.</span></span>
13. <span data-ttu-id="e68b2-563">选择 "**保存**" 以保存新控件。</span><span class="sxs-lookup"><span data-stu-id="e68b2-563">Select **Save** to save the new control.</span></span>

### <a name="export-a-template-to-json"></a><span data-ttu-id="e68b2-564">将模板导出到 JSON</span><span class="sxs-lookup"><span data-stu-id="e68b2-564">Export a Template to JSON</span></span>

<span data-ttu-id="e68b2-565">合规性管理器 (预览版) 还支持将模板导出为 JavaScript 对象表示法 (JSON) 格式。</span><span class="sxs-lookup"><span data-stu-id="e68b2-565">Compliance Manager (Preview) also supports exporting Templates to JavaScript Object Notation (JSON) format.</span></span> <span data-ttu-id="e68b2-566">这使您可以与支持 JSON 的其他系统交换合规性管理器数据。</span><span class="sxs-lookup"><span data-stu-id="e68b2-566">This enables you to exchange Compliance Manager data with other systems that support JSON.</span></span>

## <a name="reports"></a><span data-ttu-id="e68b2-567">报表</span><span class="sxs-lookup"><span data-stu-id="e68b2-567">Reports</span></span>

<span data-ttu-id="e68b2-568">您可以将评估导出到您组织中的合规性利益干系人或外部审计员和主管机构的 Excel 文件。</span><span class="sxs-lookup"><span data-stu-id="e68b2-568">You can export an Assessment to an Excel file for compliance stakeholders in your organization or for external auditors and regulators.</span></span> <span data-ttu-id="e68b2-569">报告是在导出的日期和时间进行评估的快照。</span><span class="sxs-lookup"><span data-stu-id="e68b2-569">The report is a snapshot of the Assessment as of the date and time of the export.</span></span> <span data-ttu-id="e68b2-570">该报告包含 Microsoft 和客户托管的控件的详细信息, 用于评估、控制实施状态、控制测试日期、测试结果以及指向已上载证据文档的链接。</span><span class="sxs-lookup"><span data-stu-id="e68b2-570">The report contains the details for Microsoft and customer-managed controls for the Assessment, control implementation status, control test date, test results, and links to uploaded evidence documents.</span></span> <span data-ttu-id="e68b2-571">应在存档评估之前将其导出, 因为存档的评估不会保留指向已上载文档的链接。</span><span class="sxs-lookup"><span data-stu-id="e68b2-571">You should export Assessments before archiving them because archived Assessments do not retain links to uploaded documents.</span></span>

### <a name="export-an-assessment"></a><span data-ttu-id="e68b2-572">导出评估</span><span class="sxs-lookup"><span data-stu-id="e68b2-572">Export an Assessment</span></span>

1. <span data-ttu-id="e68b2-573">在合规性管理器仪表板中, 选择 "**控件信息**" 选项卡。</span><span class="sxs-lookup"><span data-stu-id="e68b2-573">On the Compliance Manager dashboard, select **Controls Info** tab.</span></span>
2. <span data-ttu-id="e68b2-574">在下拉菜单中选择要导出的评估的组和评估。</span><span class="sxs-lookup"><span data-stu-id="e68b2-574">Select the Group and Assessment in the dropdown menus for the Assessment you want to export.</span></span>
3. <span data-ttu-id="e68b2-575">选择 "导出"。</span><span class="sxs-lookup"><span data-stu-id="e68b2-575">Select Export.</span></span> <span data-ttu-id="e68b2-576">将评估导出作为 Excel 文件下载。</span><span class="sxs-lookup"><span data-stu-id="e68b2-576">The Assessment export is downloaded as an Excel file.</span></span>

![合规性管理器评估 Excel 报告](media/compliance-manager-assessment-report.png)

## <a name="permissions"></a><span data-ttu-id="e68b2-578">权限</span><span class="sxs-lookup"><span data-stu-id="e68b2-578">Permissions</span></span>

<span data-ttu-id="e68b2-579">下表介绍了每个合规性管理器权限及其允许用户执行的操作。</span><span class="sxs-lookup"><span data-stu-id="e68b2-579">The following table describes each Compliance Manager permission and what it allows the user do.</span></span> <span data-ttu-id="e68b2-580">该表还指示分配了每个权限的角色。</span><span class="sxs-lookup"><span data-stu-id="e68b2-580">The table also indicates the role that each permission is assigned.</span></span>

||<span data-ttu-id="e68b2-581">**合规性管理器读者**</span><span class="sxs-lookup"><span data-stu-id="e68b2-581">**Compliance Manager Reader**</span></span>|<span data-ttu-id="e68b2-582">**合规性管理器参与者**</span><span class="sxs-lookup"><span data-stu-id="e68b2-582">**Compliance Manager Contributor**</span></span>|<span data-ttu-id="e68b2-583">**合规性管理器评估员**</span><span class="sxs-lookup"><span data-stu-id="e68b2-583">**Compliance Manager Assessor**</span></span>|<span data-ttu-id="e68b2-584">**合规性管理器管理员**</span><span class="sxs-lookup"><span data-stu-id="e68b2-584">**Compliance Manager Administrator**</span></span>|<span data-ttu-id="e68b2-585">**门户管理员**</span><span class="sxs-lookup"><span data-stu-id="e68b2-585">**Portal Admin**</span></span>|
|:-----|:-----|:-----|:-----|:-----|:-----|
|<span data-ttu-id="e68b2-586">**读取数据:** 用户可以读取但不能编辑数据 (模板数据和租户管理除外)。</span><span class="sxs-lookup"><span data-stu-id="e68b2-586">**Read data:** Users can read but not edit data (except for Template data and Tenant Management).</span></span>  <br> | <span data-ttu-id="e68b2-587">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-587">X</span></span> | <span data-ttu-id="e68b2-588">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-588">X</span></span> | <span data-ttu-id="e68b2-589">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-589">X</span></span> | <span data-ttu-id="e68b2-590">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-590">X</span></span>  | <span data-ttu-id="e68b2-591">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-591">X</span></span> |
|<span data-ttu-id="e68b2-592">**编辑数据:** 除了 "测试结果" 和 "测试日期" 字段 ("模板数据" 和 "租户管理" 除外) 之外, 用户可以编辑所有字段。</span><span class="sxs-lookup"><span data-stu-id="e68b2-592">**Edit data:** Users can edit all fields, except the Test Result and Test Date fields (except for Template data and Tenant Management).</span></span>  <br> || <span data-ttu-id="e68b2-593">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-593">X</span></span> | <span data-ttu-id="e68b2-594">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-594">X</span></span>  | <span data-ttu-id="e68b2-595">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-595">X</span></span> | <span data-ttu-id="e68b2-596">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-596">X</span></span> |
|<span data-ttu-id="e68b2-597">**编辑测试结果:** 用户可以编辑 "测试结果" 和 "测试日期" 字段。</span><span class="sxs-lookup"><span data-stu-id="e68b2-597">**Edit test results:** Users can edit the Test Result and Test Date fields.</span></span>  <br> ||| <span data-ttu-id="e68b2-598">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-598">X</span></span> | <span data-ttu-id="e68b2-599">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-599">X</span></span> | <span data-ttu-id="e68b2-600">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-600">X</span></span> |
|<span data-ttu-id="e68b2-601">**管理评估:** 用户可以创建、存档和删除评估。</span><span class="sxs-lookup"><span data-stu-id="e68b2-601">**Manage assessments:** Users can create, archive, and delete Assessments.</span></span>  <br> |||| <span data-ttu-id="e68b2-602">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-602">X</span></span> | <span data-ttu-id="e68b2-603">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-603">X</span></span> |
|<span data-ttu-id="e68b2-604">**管理主数据:** 用户可以查看、编辑和删除模板数据和租户管理数据。</span><span class="sxs-lookup"><span data-stu-id="e68b2-604">**Manage master data:** Users can view, edit, and delete template data and tenant management data.</span></span>  <br> |||| <span data-ttu-id="e68b2-605">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-605">X</span></span> | <span data-ttu-id="e68b2-606">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-606">X</span></span> |
|<span data-ttu-id="e68b2-607">**管理用户:** 用户可以将组织中的其他用户添加到 Reader、投稿人、评估员和管理员角色。</span><span class="sxs-lookup"><span data-stu-id="e68b2-607">**Manage users:** Users can add other users in their organization to the Reader, Contributor, Assessor, and Administrator roles.</span></span> <span data-ttu-id="e68b2-608">只有组织中具有全局管理员角色的用户才可以在门户管理员角色中添加或删除用户。</span><span class="sxs-lookup"><span data-stu-id="e68b2-608">Only those users with the Global Administrator role in your organization can add or remove users from the Portal Admin role.</span></span>  <br> ||||| <span data-ttu-id="e68b2-609">X</span><span class="sxs-lookup"><span data-stu-id="e68b2-609">X</span></span> |

### <a name="guest-access"></a><span data-ttu-id="e68b2-610">来宾访问权限</span><span class="sxs-lookup"><span data-stu-id="e68b2-610">Guest access</span></span>
  
<span data-ttu-id="e68b2-611">配置合规性管理器访问后, 默认情况下, 任何没有预配角色的用户都在**来宾访问**角色中 (也是任何非组织预配帐户 (如个人 Microsoft 帐户) 的体验)。</span><span class="sxs-lookup"><span data-stu-id="e68b2-611">After Compliance Manager access is configured, any user that does not have a provisioned role is in the **Guest access** role by default (which is also the experience of any non-organization-provisioned accounts like personal Microsoft Accounts).</span></span> <span data-ttu-id="e68b2-612">来宾访问用户不具有对所有合规性管理器功能的完全访问权限。</span><span class="sxs-lookup"><span data-stu-id="e68b2-612">Guest Access users do not have full access to all Compliance Manager features.</span></span> <span data-ttu-id="e68b2-613">他们不能看到组织的任何合规性评估数据, 但他们能够使用合规性管理器查看 Microsoft 的合规性评估报告和服务信任文档。</span><span class="sxs-lookup"><span data-stu-id="e68b2-613">They are not able to see any of the organization's compliance assessment data, however they are able to use Compliance Manager to view Microsoft's compliance assessment reports and Service Trust documents.</span></span>