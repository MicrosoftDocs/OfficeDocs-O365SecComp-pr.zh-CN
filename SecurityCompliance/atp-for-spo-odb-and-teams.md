...
- ATP 将不会扫描 SharePoint Online、OneDrive for business 或 Microsoft 团队中的每个单个文件。这是设计的。通过使用共享和来宾活动事件的进程以及智能试探和威胁信号识别恶意文件, 异步扫描文件。
...
- 在 SharePoint Online、OneDrive for business 或 Microsoft 团队中标识为恶意的文件将显示在[office 365 高级威胁防护](view-reports-for-atp.md)和威胁资源管理器 ( [office 365 威胁智能](office-365-ti.md)的一部分) 的报告中。
