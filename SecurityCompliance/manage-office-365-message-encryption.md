...
### <a name="to-manage-whether-or-not-to-allow-recipients-to-use-social-ids-to-sign-in-to-the-ome-portal"></a>若要管理允许收件人社交 Id 用于登录到 OME 门户
...
### <a name="to-manage-whether-or-not-one-time-pass-codes-are-generated-for-ome"></a>若要管理为 OME 生成一次性密码
...
### <a name="to-manage-whether-or-not-the-protect-button-appears-in-outlook-on-the-web"></a>若要管理保护按钮显示在 web 上的 Outlook
  
1. 使用 Office 365 组织中具有全局管理员权限的工作或学校帐户，请启动 Windows PowerShell 会话并连接到 Exchange Online。有关说明，请参阅[Connect to Exchange Online PowerShell 中](https://aka.ms/exopowershell)。

2. 运行带有-SimplifiedClientAccessEnabled 参数 Set-irmconfiguration cmdlet:
...
有关详细信息，以及客户端体验的视图，请参阅[查看加密邮件 iPhone 或 iPad 上](https://support.office.com/article/4d631321-0d26-4bcc-a483-d294dd0b1caf)。
  
### <a name="to-manage-whether-or-not-ios-mail-app-users-can-view-messages-protected-by-office-365-message-encryption"></a>若要管理 iOS 邮件应用程序用户可以查看受 Office 365 邮件加密的邮件
  
1. 使用 Office 365 组织中具有全局管理员权限的工作或学校帐户，请启动 Windows PowerShell 会话并连接到 Exchange Online。有关说明，请参阅[Connect to Exchange Online PowerShell 中](https://aka.ms/exopowershell)。

2. 运行带有 AllowRMSSupportForUnenlightenedApps 参数集 ActiveSyncOrganizations cmdlet:
   ...
   或者，若要配置服务无法将解密的邮件发送到 unenlightened 应用程序：
...
如果您选择不允许解密电子邮件附件，这是默认值，则用户将收到一条消息，它们没有要查看附件的权限。
...
### <a name="to-manage-whether-or-not-email-attachments-are-decrypted-on-download-from-a-web-browser"></a>若要管理电子邮件附件将解密在从 web 浏览器的下载
  
1. 使用 Office 365 组织中具有全局管理员权限的工作或学校帐户，请启动 Windows PowerShell 会话并连接到 Exchange Online。有关说明，请参阅[Connect to Exchange Online PowerShell 中](https://aka.ms/exopowershell)。

2. 使用 DecryptAttachmentFromPortal 参数运行 Set-irmconfiguration cmdlet:
...
2. 如果启用在 web 上的 Outlook 中的**保护**按钮，请通过运行带有 SimplifiedClientAccessEnabled 参数 Set-irmconfiguration cmdlet 禁用它。否则，跳过此步骤。

   ```powershell
   Set-IRMConfiguration -SimplifiedClientAccessEnabled $false
   ```

3. 通过运行带有 AzureRMSLicensingEnabled 参数设置为 false 的 Set-irmconfiguration cmdlet 禁用 OME 的新功能：
