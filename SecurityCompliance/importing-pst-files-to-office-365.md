...
4. **创建 pst 导入作业**-下一步是在 Security _AMP_ 合规性中心的 "**导入**" 页上创建 pst 导入作业, 并提交在上一步中创建的 pst 导入映射文件。 对于网络上载 (由于已将 PST 文件上传到 Azure), Office 365 将分析 PST 文件中的数据, 然后为您提供一个设置筛选器的机会, 该筛选器控制实际导入到 PST 导入映射文件中指定的邮箱的数据。 
