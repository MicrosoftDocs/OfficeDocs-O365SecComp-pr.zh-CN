...
- **存储逻辑损坏**-以用户不期望的方式添加、删除或操作数据。这些情况通常是由第三方应用程序导致的。通常情况下, 仅当用户将其视为损坏时, 才会损坏。Exchange 存储将生成逻辑损坏的事务视为一系列有效的 MAPI 操作。Exchange Online 中的[就地保留](https://docs.microsoft.com/exchange/security-and-compliance/create-or-remove-in-place-holds)功能提供了对存储逻辑损坏的保护 (因为它会阻止用户或应用程序永久删除内容)。 
