...
|PCL|邮件的网络钓鱼可信度 (PCL), 指示它是否为网络钓鱼邮件。此状态可作为以下数值之一返回:<br/>• **0-3**: 邮件的内容不太可能是网络钓鱼。 <br/>• **4-8**: 邮件的内容可能是网络钓鱼。 <br/>• **-9990**: (仅限 Exchange Online Protection) 邮件的内容可能是网络钓鱼。  <br/>  这些值用于确定您的电子邮件客户端对邮件所采取的操作。例如, Outlook 使用 PCL 标记来阻止可疑邮件的内容。有关网络钓鱼以及 Outlook 如何处理网络钓鱼邮件的详细信息, 请参阅[打开或关闭电子邮件中的链接](https://support.office.com/article/2D79B907-93B6-4774-82E6-1F0385CF20F8)。|
...
|**标头字段**|**说明**|
|:-----|:-----|
|spf|说明邮件的 SPF 检查结果。可能的值包括：  <br/>• **pass (IP 地址)**: 指示已传递邮件的 SPF 检查, 并包括发件人的 IP 地址。客户端有权代表发件人的域发送或中继电子邮件。<br/>•**故障 (IP 地址)**: 指示邮件的 SPF 检查失败, 并包括发件人的 IP 地址。这有时称为_硬失败_。<br/>• **softfail (原因)**: 指示 SPF 记录已将主机指定为不允许发送但处于转换状态。 <br/>•无**特定**说明: 指示 SPF 记录已明确声明它不会断言 IP 地址是否已获得授权。 <br/>•**无**: 指示域没有 SPF 记录或 spf 记录未计算结果。 <br/>• **temperror 出现**: 指示发生了可能是暂时临时错误的错误, 例如, DNS 错误。如果不进行任何管理员操作, 稍后可能会成功。<br/>• **permerror**: 指示发生了永久性错误。例如, 当域具有格式不正确的 SPF 记录时, 会发生这种情况。|
|5321.mailfrom|包含发送邮件的源域。此电子邮件的任何错误都将发送到负责此域的 postmaster 或实体。在邮件信封上有时也称其为"5321.MailFrom 地址"或"反向路径地址"。|
|dkim|说明邮件的 DKIM 检查结果。可能的值包括：  <br/>• **pass**: 指示对传递的邮件的 DKIM 检查。 <br/>•**失败 (原因)**: 指示邮件的 DKIM 检查失败以及原因。例如, 如果邮件未签名或签名未验证。<br/>•**无**: 指示邮件未签名。这可能会 (也可能不表示) 域具有 DKIM 记录, 或者 DKIM 记录未计算结果, 只是该邮件未签名。|
|头. d|DKIM 签名中标识的域（如有）。这指的是 针对公钥查询的域。|
|dmarc|说明邮件的 DMARC 检查结果。可能的值包括：  <br/>• **pass**: 指示对传递的邮件的 DMARC 检查。 <br/>•**失败**: 指示邮件的 DMARC 检查失败。 <br/>• **bestguesspass**: 指示域不存在任何 DMARC TXT 记录, 但如果其中一个存在, 则邮件的 DMARC 检查将通过。这是因为5321地址中的域与5322.from 地址中的域相匹配。<br/>•**无**: 指示 DNS 中不存在发送域的任何 DKIM TXT 记录。|
|action|指示垃圾邮件筛选器基于 DMARC 检查结果执行的操作。例如：  <br/>• **permerror**: 在 DMARC 评估期间发生永久错误, 例如, 在 DNS 中遇到格式不正确的 DMARC TXT 记录。尝试重新发送此邮件不太可能以不同的结果结束。相反, 您可能需要联系域的所有者才能解决问题。<br/>• **temperror 出现**: DMARC 评估期间发生临时错误。您可能可以请求发件人稍后重新发送邮件, 以便正确处理电子邮件。<br/>• **oreject**或**o. 拒绝**: 代表替代拒绝。在这种情况下, Office 365 在收到一条消息, 该邮件从其 DMARC TXT 记录的策略为 p = 拒绝的域中收到一条 DMARC 检查失败的消息时使用此操作。Office 365 将邮件标记为垃圾邮件, 而不是删除或拒绝邮件。若要详细了解为什么要以这种方式配置 office 365, 请参阅[office 365 如何处理 DMARC 失败的入站电子邮件](use-dmarc-to-validate-email.md#inbounddmarcfail)。<br/>• **pct**: 指明不传递 DMARC 的邮件百分比小于 100% 的邮件仍将传递。这意味着邮件失败 DMARC, 并且该策略已设置为 "隔离", 但 "pct" 字段未设置为 100%, 并且系统根据指定的域策略随机确定不应用 DMARC 操作。<br/>•**百分比拒绝**: 指示仍将传递小于 100% 且未传递 DMARC 的邮件的百分比。这意味着邮件失败 DMARC, 并且该策略已设置为 "拒绝", 但 "pct" 字段未设置为 100%, 并且系统根据指定的域策略随机确定不应用 DMARC 操作。|
|标头。从|电子邮件邮件头中 "发件人" 地址的域。这有时称为 " _5322.from_地址"。|
