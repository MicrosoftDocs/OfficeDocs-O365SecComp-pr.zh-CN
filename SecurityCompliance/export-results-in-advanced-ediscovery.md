...
6. 单击**修改**设置进行审阅字段设置。 
   ...
   - 在**查看字段设置**下**选择方案**的下拉列表中，选择的方案和查看范围。根据您选择要显示的设置。
    
      - **查看所有**（默认值）： 默认情况下选中所有电子邮件、 附件和文档。 
    
      - **查看一组中的所有唯一内容**： Inclusives 和唯一的非独占副本，电子邮件中的唯一附件设置级别，代表从每个确切的重复项集。
    
      - **查看一组-不含副本中的所有唯一内容**： Inclusives，电子邮件中的唯一附件设置级别，代表从每个确切的重复项集。
    
      - **查看所有的唯一内容及相关的系列文件**： Inclusives，电子邮件设置级别，来自每个完全重复集代表中唯一附件扩展以包含系列文件。
    
      - **自定义**（允许您在对话框中定义的选项）： 默认值是保留当前选定内容并启用所有对话框选项，以允许其所选内容。如果选择此选项，可以然后自定义电子邮件、 文档、 附件的设置和 miscellaneous。
    
    - 在**电子邮件**，下选择要导出的电子邮件。
    
      - **所有电子邮件**: （默认值） 选择所有电子邮件。
    
      - **Inclusives**： 非独占电子邮件的线程，最后一个电子邮件，并且包含所有其他电子邮件从线程。
    
      - **Inclusives 和唯一的非独占副本**： 非独占副本和 inclusives 具有相同主题、 正文和附件;唯一的非独占副本是唯一的这些电子邮件的副本。
    
    - 在**文档**下选择要导出的文档。 
    
      - **所有文档**: （默认值） 选择所有文档。
    
      - **数据透视表**： 选择的代表近乎重复项集，查看设置时通常使用为基线的文件。
    
      - **从每个完全重复集代表**： 唯一近乎重复文件 （包括数据透视表）。
    
    - 在**附件**下选择要导出的附件。 
    
      - **所有附件**: （默认值） 选择所有附件。
    
      - **唯一附件事例级别**： 内指定用例的唯一的附件文件。
    
      - **电子邮件中的唯一附件设置级别**： 唯一的附件文件中指定的电子邮件大小写。
    
   - 下**Micellaneous**，您可以选择**将文档作为附件**、**将文档作为电子邮件**，或**展开，包括家庭文件**。当您选择**扩展以包含系列文件**，供审阅标记每个文件时，将同时也标记同一系列的所有文件。
    
7. 选择**保存**以保存设置。 
    
8. 指定导出参数后，若要开始导出批次单击**创建导出会话**。
    
    导出期间，将状态显示在**任务状态**。在**导出摘要**显示结果。
    
9. 在**文件下载**窗口中，单击**复制到剪贴板**复制导出密钥。 
...
11. 在**电子数据展示导出工具**：
    ...
    -  在**粘贴将用于连接到源的共享访问签名**，导出密钥该 youcopied 到剪贴板中粘贴步骤 7。
    
    - 单击**浏览**，选择用于存储在本地计算机上下载的导出文件的目标位置。 
    
    - 单击**启动**。导出文件下载到本地计算机。如果您在步骤 4 中选择**导出到用户定义的 Azure blob** ，会话将导出到您选择的 Blob 存储 URL 目标。
