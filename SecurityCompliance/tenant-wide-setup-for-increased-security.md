...
|仪表板 * * * *|****Description****|
|:-----|:-----|
|威胁管理仪表板  <br/> |在安全&amp;合规中心的 "威胁管理" 部分中, 使用此仪表板查看已处理的威胁, 并作为一种方便的工具, 用于向业务决策制定者报告已完成的威胁智能, 以保护您的知情.  <br/> |
...
|区域 * * * *|包含默认策略 * * * *|建议 * * * *|
|:-----|:-----|:-----|
|**共享**(SharePoint Online 和 OneDrive for business)|可访问|默认情况下启用外部共享。建议使用以下设置:<br/>•允许与经过身份验证的外部用户共享, 并使用匿名访问链接 (默认设置)。 <br/>  •匿名访问链接将在这几天内过期。如果需要, 请输入一个数字, 如30天。<br/>•默认链接类型-选择 "内部" (仅限组织中的 "人员")。希望使用匿名链接进行共享的用户必须从 "共享" 菜单中选择此选项。<br/><br/> 详细信息:[外部共享概述](https://support.office.com/article/c8a462eb-0723-4b0b-8d0a-70feafe4be85)|
