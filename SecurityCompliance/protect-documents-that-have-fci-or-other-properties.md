...
例如, 您的组织可能会使用 Windows Server FCI 识别包含个人身份信息 (PII) 的文档, 如社会安全号码, 然后通过设置**个人身份信息**对文档进行分类。属性设为 "**高**"、"**中等**"、"**低**"、"**公共**" 或 "**不是 pii** ", 基于在文档中找到的 PII 的类型和次数。在 Office 365 中, 可以创建一个 DLP 策略, 用于标识将该属性设置为特定值 (如 "**高**" 和 "**中**") 的文档, 然后执行诸如阻止对这些文件的访问等操作。如果将该属性设置为 "**低**" (如发送电子邮件通知), 则同一策略可以具有另一条规则, 以执行其他操作。通过这种方式, Office 365 中的 DLP 与 windows server FCI 集成, 并可帮助保护从基于 Windows server 的文件服务器上传或共享到 office 365 的 office 文档。
  
DLP 策略仅查找特定的属性名称/值对。只要属性具有对应的 SharePoint 搜索的托管属性, 就可以使用任何 document 属性。例如, SharePoint 网站集可能使用名为 "**客户**" 的必填字段的 "**行程报告**" 的内容类型。只要某人创建了行程报告, 他们就必须输入客户名称。此外, 还可以在 DLP 策略中使用此属性名称/值对, 例如, 如果您想要一条规则, 以便在 "**客户**" 字段包含**Contoso**时为外部用户阻止对文档的访问。
...
您需要先在 SharePoint 管理中心中创建托管属性, 然后才能在 DLP 策略中使用 Windows Server FCI 属性或其他属性。下面介绍了原因。
...
2. 在左侧导航中, 选择 "**管理中心** \> " " **SharePoint**"。你现在位于 SharePoint 管理中心。
...
一个规则阻止访问 "**个人可识别信息**" 属性等于 "**高**" 或 "**中等**" 的内容。第二条规则发送有关**个人可识别信息**属性等于 "**低**" 的内容的通知。
...
> [!CAUTION]
> 对网站重新编制索引可能会导致搜索系统出现大量负载。除非您的方案确实需要, 否则不要对网站重新编制索引。 
